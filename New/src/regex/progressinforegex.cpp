...
void ProgressInfoRegex::setupPatterns() {
	_indexer << QRegularExpression(QString("frame=\\s*([0-9]*)")); // frame
	_indexer << QRegularExpression(QString("fps=\\s*([0-9]*\\.?[0-9]?)")); // fps
	_indexer << QRegularExpression(QString("bitrate=\\s*([0-9]*\\.?[0-9]?)kbits/s")); // bitrate
	_indexer << QRegularExpression(QString("time=\\s*([0-9]*):([0-9]*):([0-9]*)\\.([0-9]*)")); // processed time
}
