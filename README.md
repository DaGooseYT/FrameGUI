...
3. The values in the green box can remain on their defaults, unless you need more memory or threads to AviSynth+. The maximum memory value unit is MB. The maximum amount of threads you can use is 16 and the maximum amount of memory you can allow is 4GB (4096 MB). If you already know the syntax of the SVPFlow parameters, skip steps 4, 5, 6 and enter the parameters in the `Smooth`, `Super`, and `Analyse` text boxes
